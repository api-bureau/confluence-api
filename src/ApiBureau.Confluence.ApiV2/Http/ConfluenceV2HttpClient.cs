...
public sealed class ConfluenceV2HttpClient
{
    private const string ApiPrefix = "wiki/api/v2";

    private readonly HttpClient _client;
    private readonly ILogger<ConfluenceV2HttpClient> _logger;
    private readonly JsonSerializerOptions _jsonOptions = new(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    };

    public ConfluenceV2HttpClient(HttpClient client, ILogger<ConfluenceV2HttpClient> logger)
    {
        _client = client;
        _logger = logger;
    }
    ...
    public Task<PagedV2Response<T>?> GetPageAsync<T>(string path, CancellationToken token = default)
        => GetAsync<PagedV2Response<T>>(path, token);
    ...
    private static async Task EnsureSuccessAsync(HttpResponseMessage response, CancellationToken token)
    {
        ...
        throw new ConfluenceV2ApiException(response.StatusCode, response.ReasonPhrase, responseBody);
    }
}